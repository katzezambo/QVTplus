...
function [Anatpath,APpath,LRpath,SIpath] = retFlowFolders(path2flow,Vendor,varargin)
    TestDir = dir(path2flow);
    TestDir = TestDir(~ismember({TestDir.name}, {'.', '..'}));  % Filter out '.' and '..'
    ...
    paths = cell(1, 4);  % Use a cell array to store paths for efficiency
    
    if strcmp('GE', Vendor)
        for i = 1:length(TestDir)
            Exp = TestDir(i).name;
            for j = 1:length(RegExp)
                match = regexp(Exp, RegExp{j}, 'match');
                if ~isempty(match) && isempty(paths{j})
                    paths{j} = fullfile(path2flow, Exp);
                    % Optionally break here if you only want the first match
                    % break;
                end
            end
        end
    elseif strcmp('Siemens', Vendor)
        paths = {fullfile(path2flow, 'M1'), fullfile(path2flow, 'P3'), fullfile(path2flow, 'P2'), fullfile(path2flow, 'P1')};
    end
    
    [Anatpath, APpath, LRpath, SIpath] = deal(paths{:});  % Unpack paths from cell array
end